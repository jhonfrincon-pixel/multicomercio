...
function InventorySection() {
  ...
  const handlePriceChange = async (id: string, newPrice: string) => {
    ...
    if (!isNaN(price)) {
      try {
        await updateProduct(id, { price } as Partial<Product>);
      } catch (error) {
        toast.error('Error al actualizar el precio en el servidor');
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEditMode && productToEdit) {
        await updateProduct(productToEdit.id, productData as Product);
